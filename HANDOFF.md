...
- **Corner-peel stuck-state bug:** the original hover implementation used
  `pointerenter`/`pointerleave` on the small 34px peel boxes. On fast
  mouse movement, `leave` could fail to fire, leaving the *wrong* corner
  stuck visible (exactly what Yossi reported: "cursor on the left, right
  corner lifts"). Fixed by switching to continuous proximity tracking on
  every `pointermove` — nearest-corner-within-90px, recomputed from
  scratch each time, self-correcting. Stress-tested with a 40-sample
  continuous sweep across the full page width with no flicker/stuck
  states.
